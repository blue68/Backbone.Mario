describe("renderer", function(){
  ...
  describe("when given a template id to render", function(){
    ...
    beforeEach(function(){
      ...
      spyOn(Backbone.Mario.TemplateCache, "get").andCallThrough();
      var html = Backbone.Mario.Renderer.render(templateSelector);
      result = $(html);
    });

    it("should retrieve the template from the cache", function(){
      expect(Backbone.Mario.TemplateCache.get).toHaveBeenCalledWith(templateSelector);
    });
  ...
  describe("when given a template and data to render", function(){
    ...
    beforeEach(function(){
      ...
      var data = {foo: "bar"}
      var html = Backbone.Mario.Renderer.render(templateSelector, data);
      result = $(html);
    });
  ...
  describe("when no template is provided", function(){
    ...
    beforeEach(function(){
      render = _.bind(Backbone.Mario.Renderer.render, Backbone.Mario.Renderer);
    });
  ...
  describe("when overriding the `render` method", function(){
    ...
    beforeEach(function(){
      oldRender = Backbone.Mario.Renderer.render;

      Backbone.Mario.Renderer.render = function(template, data){
        return "<foo>custom</foo>";
      };

      result = Backbone.Mario.Renderer.render("", {});
      result = $(result);
    });

    afterEach(function(){
      Backbone.Mario.Renderer.render = oldRender;
    });
  ...
  describe("when providing a precompiled template", function(){
    it("should use the provided template function", function(){
      ...
      var result = Backbone.Mario.Renderer.render(templateFunction,{foo : 'bar'});
