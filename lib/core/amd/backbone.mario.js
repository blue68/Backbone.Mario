/*! Backbone.Mario - v1.0.0 - 2013-01-06
* https://github.com/qtalen/Backbone.Mario
* Copyright (c) 2013 peng.qianp; Licensed  */
(function (root, factory) {
  ...
  Backbone.Mario = Mario = (function(Backbone, _, $){
    var Mario = {};
  ...
  Mario.EventBinder = Backbone.EventBinder.extend({
  
    augment: function(target){
      var eventBinder = new Mario.EventBinder();
  ...
  Mario.addEventBinder = function(target){
    var eventBinder = new Mario.EventBinder();
    target.eventBinder = eventBinder;
    target.bindTo = _.bind(eventBinder.bindTo, eventBinder);
    target.unbindFrom = _.bind(eventBinder.unbindFrom, eventBinder);
  ...
  // `Deferred` object.
  Mario.Callbacks = function(){
    this._deferred = $.Deferred();
    this._callbacks = [];
  };
  
  _.extend(Mario.Callbacks.prototype, {
  ...
  Mario.TemplateCache = function(templateId){
  ...
  _.extend(Mario.TemplateCache, {
    ...
    get: function(templateId){
      ...
      if (!cachedTemplate){
        cachedTemplate = new Mario.TemplateCache(templateId);
  ...
  _.extend(Mario.TemplateCache.prototype, {
  ...
  Mario.Region = function(options){
    this.options = options || {};
  
    Mario.addEventBinder(this);
  
    this.el = Mario.getOption(this, "el");
  ...
  _.extend(Mario.Region.prototype, Backbone.Events, {
    ...
    show: function(view){
      ...
      Mario.triggerMethod.call(view, "show");
      Mario.triggerMethod.call(this, "show", view);
    ...
    close: function(){
      ...
      Mario.triggerMethod.call(this, "close");
  ...
  Mario.Region.extend = Mario.extend;
  
  // Marionette.View
  // ---------------
  
  // The core view type that other Marionette views extend from.
  Mario.View = Backbone.View.extend({
    ...
    triggerMethod: Mario.triggerMethod,
    ...
    getTemplate: function(){
      return Mario.getOption(this, "template");
    },
  ...
  Mario.ItemView =  Mario.View.extend({
    constructor: function(){
      Mario.View.prototype.constructor.apply(this, arguments);
    ...
    close: function(){
      ...
      Mario.View.prototype.close.apply(this, arguments);
  ...
  Mario.CollectionView = Mario.View.extend({
    constructor: function(options){
      this.initChildViewStorage();
  
      Mario.View.prototype.constructor.apply(this, arguments);
      this.initialEvents();
      this.onShowCallbacks = new Mario.Callbacks();
    ...
    showEmptyView: function(){
      var EmptyView = Mario.getOption(this, "emptyView");
    ...
    getItemView: function(item){
      var itemView = Mario.getOption(this, "itemView");
  ...
  Mario.CompositeView = Mario.CollectionView.extend({
    constructor: function(options){
      Mario.CollectionView.apply(this, arguments);
    ...
    // has been defined
    getItemView: function(item){
      var itemView = Mario.getOption(this, "itemView") || this.constructor;
    ...
    renderCollection: function(){
      Mario.CollectionView.prototype.render.apply(this, arguments);
    ...
    renderModel: function(){
      ...
      var template = this.getTemplate();
      return Mario.Renderer.render(template, data);
    },
  ...
  Mario.Layout = Mario.ItemView.extend({
    regionType: Mario.Region,
    ...
    constructor: function () {
      ...
      Backbone.Mario.ItemView.apply(this, arguments);
    },
    ...
    render: function(){
      ...
      var result = Mario.ItemView.prototype.render.apply(this, arguments);
      return result;
    },
    ...
    close: function () {
      ...
      this.closeRegions();
      this.destroyRegions();
      Backbone.Mario.ItemView.prototype.close.call(this, arguments);
    },
    ...
    initializeRegions: function () {
      ...
      _.each(regions, function (region, name) {
        ...
        var regionManager = Mario.Region.buildRegion(region, that.regionType);
  ...
  Mario.FormView = Mario.ItemView.extend({
  	...
  	changeFieldVal : function(model, fields) {
        ...
        if(domItem) {
          domItem.val(this.model.get(modelProperty));
        }
      },
      ...
      serializeFormData : function () {
        ...
        function getFieldData(options,field) {
          ...
          if (el) {
              data[field] = this.getInputVal(el);
          }
        }
      ...
      beforeFormSubmit : function (e) {
        ...
        if (success) {
          if (_.isFunction(this.onSubmit)) {
              return this.onSubmit.apply(this, [e]);
          }
        } else {
          if (_.isFunction(this.onSubmitFail)) {
              this.onSubmitFail.apply(this, [errors]);
          }
      ...
      handleFieldEvent : function(evt, eventName) {
        ...
        if (fieldOptions && fieldOptions.validateOn === eventName) {
          ...
          if (!_.isEmpty(errors) && _.isFunction(this.onValidationFail)) {
      ...
      validateRule : function (val,validationRule) {
        ...
        if (!validationRule) {
          throw new Error('Not passed a validation to test');
        }
  
        if (validationRule === 'required') {
          return Mario.FormView.Validator.required(val);
        }
        ...
        if (this.rules && this.rules[validationRule]) {
          ...
          return _(Mario.FormView.Validator.validate).bind(this)(validationRule, val, options);
        }
        return true;
      },
  ...
  Mario.FormView.Validator = {
      ...
      validate : function(validator, val, options) {
        if (_.isFunction(Mario.FormView.Validator[validator])) {
          return _(Mario.FormView.Validator[validator]).bind(this)(val,options);
        }
      ...
      alpha : function(val) {
        return Mario.FormView.Validator.regex.alpha.test(val);
      },
  
      alphanum : function (val) {
        return Mario.FormView.Validator.regex.alphanum.test(val);
      },
  
      email : function(val) {
        return Mario.FormView.Validator.regex.email.test(val);
      },
  
      required : function(val) {
        if (val === false || _.isNull(val) || _.isUndefined(val) ||  (_.isString(val) && val.length === 0)) {
          return false;
        }
        return true;
      },
  ...
  Mario.AppRouter = Backbone.Router.extend({
    ...
    constructor: function(options){
      ...
      if (this.appRoutes){
        var controller = Mario.getOption(this, "controller");
  ...
  // Mario applications
  Mario.Module = function(moduleName, app){
  ...
  _.extend(Mario.Module.prototype, Backbone.Events, {
    ...
    _setupInitializersAndFinalizers: function(){
      this._initializerCallbacks = new Mario.Callbacks();
      this._finalizerCallbacks = new Mario.Callbacks();
    }
  });
  
  // Function level methods to create modules
  _.extend(Mario.Module, {
    ...
    _getModuleDefinition: function(parentModule, moduleName, app){
      ...
      if (!module){
        ...
        module = new Mario.Module(moduleName, app);
